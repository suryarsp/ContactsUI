...
constructor(props: Readonly<{}>) {
  ...
  render () {
    const displayContacts = this.state.displayContacts.filter(c => !c.isLoggedIn);
    const { contacts, loggedInContact } = this.state;
    return (
      ...
      <div className="col sidemenuContainer">
        <a className="topMenu" onClick={(e) => {e.preventDefault();}}>
          <img width="20" src={menu} />
        </a>
        <ul className="sideMenu">
          <li className="active">
            <a onClick={(e) => {e.preventDefault();}}>
              <img width="20" src={person} />
            </a>
          </li>
          <li>
            <a onClick={(e) => {e.preventDefault();}}>
              <img width="20" src={sheet} />
            </a>
          </li>
          <li>
            <a onClick={(e) => {e.preventDefault();}}>
              <img width="20" src={clock} />
            </a>
          </li>
          <li>
            <a onClick={(e) => {e.preventDefault();}}>
              <img width="20" src={storage} />
            </a>
          </li>
          <li>
            <a onClick={(e) => {e.preventDefault();}}>
            ...
            <div className="col-md-9 p-0">
              <div className="searchContainer h-100">
                <span className="searchIcon">
                  <img width="10" src={search} />
                </span>
                <input className="form-control" type="text" />
              </div>
            </div>
            <div className="col-md-3 p-0 d-flex justify-content-end">
              <a onClick={(e) => {e.preventDefault();}} className="profileAction">
                <img width="10" src={plus} />
                Add
              </a>
              <a onClick={(e) => {e.preventDefault();}} className="profileAction">
                <img width="15" src={mail} />
              </a>
              <span>
              <select name="Select login contact" value={loggedInContact.contactId} onChange={e => this.onChangeLoginContact(e.target.value)}>
                {
                  contacts.map((c) => (
